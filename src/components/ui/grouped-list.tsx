...
export function GroupedList({
  ...
  return (
    <div className={cn("space-y-1", className)}>
      ...
      <div
        className="rounded-neu-lg overflow-hidden divide-y divide-[var(--separator)] shadow-neu-sm"
...
export function GroupedListItem({
  ...
  return (
    ...
    >
      {leftIcon && (
        <span className="flex-shrink-0 w-7 text-center">{leftIcon}</span>
      )}
...
export function GroupedListCell({
  ...
  return (
    <GroupedListItem
      ...
      rightContent={
        value ? (
          <span className="text-[var(--text-secondary)] text-[17px] truncate">
            {value}
          </span>
        ) : undefined
      }
      ...
      rightContent={
        <button
          ...
          className={cn(
            ...
            checked ? "shadow-neu-sm" : "shadow-neu-inset-sm",
            disabled && "opacity-50",
          )}
          style={{
            background: checked
