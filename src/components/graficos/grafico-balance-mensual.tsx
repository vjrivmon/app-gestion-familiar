...
import { useGraficosData, COLORES_GRAFICO, type DatoBalanceMensual } from '@/hooks/use-graficos-data'
...
export function GraficoBalanceMensual({
  ...
  const datosTransformados = datos.map(d => ({
    ...d,
    vicenteColor: d.vicente >= 0 ? COLORES_GRAFICO.positivo : COLORES_GRAFICO.negativo,
    ireneColor: d.irene >= 0 ? COLORES_GRAFICO.positivo : COLORES_GRAFICO.negativo
  }))
        ...
        >
          ...
          <Tooltip
            ...
            formatter={(value: number, name: string) => {
              ...
              return [
                <span 
                  key="value"
                  style={{ color: value >= 0 ? COLORES_GRAFICO.positivo : COLORES_GRAFICO.negativo }}
                >
                  {prefix}{formatMoney(value * 100)}
                </span>,
                name === 'vicente' ? 'Vicente' : 'Irene'
              ]
            }}
          ...
          {mostrarLeyenda && (
            <Legend 
              ...
              payload={[
                { value: 'Vicente', type: 'rect', color: COLORES_GRAFICO.vicente },
                { value: 'Irene', type: 'rect', color: COLORES_GRAFICO.irene }
              ]}
            />
          )}
