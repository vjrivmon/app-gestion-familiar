...
type PrestamoInput = Omit<
  Prestamo,
  "id" | "hogar_id" | "created_at" | "pagado" | "fecha_pago"
>;
...
export function usePrestamos(): UsePrestamosReturn {
  ...
  const crearPrestamo = useCallback(
    async (data: PrestamoInput): Promise<Prestamo | null> => {
      ...
      try {
        ...
        setError(
          err instanceof Error ? err.message : "Error al crear préstamo",
        );
        return null;
      }
    },
  ...
  const eliminarPrestamo = useCallback(
    async (id: string): Promise<boolean> => {
      try {
        ...
        setError(
          err instanceof Error ? err.message : "Error al eliminar préstamo",
        );
        return false;
      }
    },
  ...
  const balanceNeto = useMemo((): BalancePrestamos => {
    ...
    return {
      ...
      deudorNombre: nombres[deudor],
      cantidad,
      resumen: `${nombres[deudor]} debe a ${nombres[acreedor]}`,
    };
  }, [prestamosPendientes]);
