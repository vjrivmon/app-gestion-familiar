...
export function useMenuSemanal() {
    ...
    try {
      ...
      if (!menuData) {
        ...
        for (let i = 0; i < 7; i++) {
          ...
          dias.push({ menu_id: nuevoMenu.id, dia: fechaStr, tipo: 'comida', receta_id: null, notas: null })
          dias.push({ menu_id: nuevoMenu.id, dia: fechaStr, tipo: 'cena', receta_id: null, notas: null })
        }
      ...
      setMenu(prev => {
        ...
        return {
          ...
          dias: prev.dias.map(d => 
            d.id === diaId 
              ? { ...d, receta_id: recetaId, receta: recetaId ? recetas.find(r => r.id === recetaId) || null : null }
              : d
          )
        }
      })
  ...
  const generarListaCompra = async () => {
    ...
    try {
      ...
      const ingredientesMap = new Map<string, { nombre: string; cantidad: string; categoria: string }>()
  ...
  const diasOrganizados = menu?.dias?.reduce((acc, dia) => {
