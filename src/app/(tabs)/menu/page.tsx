...
import { UtensilsCrossed, Plus, BookOpen, ChevronLeft, ChevronRight, Loader2, X, Clock, Users } from 'lucide-react'
...
export default function MenuPage() {
  ...
  const abrirSelector = (diaData: { id: string; dia: string; tipo: string }) => {
  ...
  return (
    <div className="min-h-screen p-4 pb-32">
      ...
      <div className="flex items-center justify-between mb-4">
        <button onClick={semanaAnterior} className="p-2 text-accent flex items-center">
          <ChevronLeft className="w-5 h-5" />
          Anterior
        </button>
        <span className="font-semibold">{rangoSemana}</span>
        <button onClick={semanaSiguiente} className="p-2 text-accent flex items-center">
          Siguiente
          ...
          return (
            <div key={dia} className={`card p-3 ${esHoy ? 'ring-2 ring-accent' : ''}`}>
              <div className="flex items-center gap-3 mb-2">
                <span className={`w-10 text-[13px] font-semibold ${esHoy ? 'text-accent' : 'text-[var(--text-secondary)]'}`}>
                  {dia}
                </span>
                <span className="text-[13px] text-[var(--text-muted)]">
                  {format(fecha, 'd MMM', { locale: es })}
                </span>
                {esHoy && <span className="text-xs bg-accent text-white px-2 py-0.5 rounded-full">Hoy</span>}
              </div>
          ...
          <div className="bg-[var(--surface-primary)] w-full rounded-t-2xl max-h-[80vh] flex flex-col">
            ...
            {diaSeleccionado && (
              <div className="px-4 py-2 bg-[var(--surface-secondary)] text-sm">
                {format(new Date(diaSeleccionado.dia), "EEEE d 'de' MMMM", { locale: es })} - {diaSeleccionado.tipo === 'comida' ? '🍽️ Comida' : '🌙 Cena'}
              </div>
            )}
